...
class ExecutiveDashboardModule:
    ...
    def _generate_key_metrics(self) -> List[BoardMetric]:
        ...
        risk_score = (
            sum(e.get("risk_score", 0) for e in risk_entities)
            / max(1, len(risk_entities))
        )
        compliance_rate = (
            100.0
            - (
                sum(1 for f in findings if f.status != "CLOSED")
                / max(1, len(findings))
                * 100
            )
        )
        detection_rate = (
            sum(1 for c in cases if c.get("status") == "resolved")
            / max(1, len(cases))
            * 100
            if cases
            else 95.0
